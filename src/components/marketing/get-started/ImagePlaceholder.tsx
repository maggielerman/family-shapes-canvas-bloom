...
export default function ImagePlaceholder({ variant = 'default', className = '' }: ImagePlaceholderProps) {
  const sizeClasses = {
    default: 'w-full h-64',
  ...
  return (
    <div className={`flex items-center justify-center ${className}`}>
      <div className={`${sizeClasses[variant]} bg-gradient-to-br from-coral-50 to-dusty-50 rounded-2xl border-2 border-dashed border-coral-200 flex flex-col items-center justify-center p-8 text-center`}>
        <ImageIcon className={`${iconSizes[variant]} text-coral-400 mb-4`} />
        <h3 className={`${textSizes[variant].title} font-medium text-coral-700 mb-2`}>Visual Placeholder</h3>
