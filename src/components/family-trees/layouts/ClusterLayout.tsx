...
export function ClusterLayout({ persons, connections, relationshipTypes, width, height, onPersonClick }: ClusterLayoutProps) {
  ...
  useEffect(() => {
    ...
    g.selectAll('.link')
      ...
      .append('path')
      .attr('class', 'link')
      .attr('d', d3.linkHorizontal<d3.HierarchyLink<Person>, ClusterNode>()
        .x(d => d.y!)
        .y(d => d.x!))
      .attr('fill', 'none')
    ...
    nodes.filter(d => d.data.profile_photo_url)
      ...
      .attr('y', d => d.children ? -15 : -20)
      .attr('width', d => d.children ? 30 : 40)
      .attr('height', d => d.children ? 30 : 40)
      .attr('href', d => d.data.profile_photo_url!)
      .attr('clip-path', d => d.children ? 'circle(15px)' : 'circle(20px)');
    ...
    // Add generation labels using calculated generations
    const generations = new Map<number, ClusterNode[]>();
    root.descendants().forEach(node => {
      const generationInfo = generationMap.get(node.data.id);
      const generation = generationInfo?.generation ?? node.depth;
    ...
    generations.forEach((nodes, generation) => {
      if (nodes.length > 0) {
        const avgY = d3.mean(nodes, d => d.x!) || 0;
        const generationColor = nodes[0] ? generationMap.get(nodes[0].data.id)?.color : undefined;
        ...
        g.append('text')
          ...
          .attr('fill', generationColor || 'hsl(var(--muted-foreground))')
          .text(`Gen ${generation}`);
      }
    });
