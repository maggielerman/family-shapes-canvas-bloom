...
import { getDonorByUserId } from "@/utils/donorUtils";
...
const DonorDashboard = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Load donor profile from donor_profiles table
      const donorData = await getDonorByUserId(user.id);
      
      if (!donorData) {
        console.log('⚠️ No donor profile found, using default values');
        // Set default values if no donor profile exists
        setStats({
          connectedFamilies: 0,
          lastHealthUpdate: null,
          profileCompleteness: 0,
      ...
      console.log('✅ Donor data loaded:', donorData);

      // Calculate profile completeness based on donor_profiles fields
      const profileFields = [
        donorData?.donor_number,
        donorData?.cryobank_name,
        donorData?.height,
        donorData?.weight,
        donorData?.eye_color,
      ...
      // TODO: Implement when donor_recipient_connections table is created
      ...
      const lastUpdate = donorData?.last_health_update 
        ? new Date(donorData.last_health_update)
        : null;
