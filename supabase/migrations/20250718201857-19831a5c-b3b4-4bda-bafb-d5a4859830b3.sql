...
ALTER TABLE public.family_tree_folders 
ADD CONSTRAINT unique_folder_name_in_parent 
UNIQUE(family_tree_id, parent_folder_id, name);

-- Create partial unique index for root folders (where parent_folder_id IS NULL)
CREATE UNIQUE INDEX unique_root_folder_name 
ON public.family_tree_folders(family_tree_id, name) 
